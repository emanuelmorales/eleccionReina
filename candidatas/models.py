...
class Foto(models.Model):
    candidatura = models.ForeignKey(Candidata, on_delete=models.CASCADE, related_name='fotos')
    imagen = models.ImageField(upload_to='fotos_candidatas/')
    descripcion = models.CharField(max_length=255, blank=True)

    def __str__(self):
        return f"Foto de {self.candidatura.nombre} - {self.id}"

class Puntuacion(models.Model):
    candidatura = models.ForeignKey(Candidata, on_delete=models.CASCADE, related_name='puntuaciones')
    ...
    def __str__(self):
        return f"Puntuacion de {self.candidatura.nombre} - Total: {self.total}"